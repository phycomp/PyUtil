...
def restoreSys():
  cmd='copy /Users/user1/AppData/Roaming/gcin/XiangJingSheng.cin.ori /Users/user/AppData/Roaming/gcin/XiangJingSheng.cin'
  print(cmd)
  system(cmd)

#fname, Ori=argv[1], '%s.ori'%argv[1]
def bkup(fname):
  # fname=Path('~/AppData/Roaming/DIME/0XiangJingSheng.cin').expanduser()
  # Ori=Path('~/AppData/Roaming/DIME/0XiangJingSheng.cin.ori').expanduser()
  # fname=Path(Ori).expanduser()
  Ori=fname.with_suffix('.ori')
  shCopy(fname, Ori)
#cmd='copy %s %s'%(fname, Ori)
#print(cmd)
#system(cmd)
def mnplXJS(fname):
