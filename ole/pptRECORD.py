...
from pptGlobals import getSignedInt, getUnsignedInt, getDouble, outputln, textdump as textDump, getUTF8FromUTF16, getTextBytes, outputZipContent, stringizeColorRef, getFloat, nulltrunc
#from stUtil import rndrCode
#rndrCode(['textDump', textDump])
#textDump=b''
# -------------------------------------------------------------------
...
"""
    ...
    def __print (self, text):
        try:
            outputln(self.prefix + text, recordType = self.recordType)
        except UnicodeEncodeError:
            pptGlobals.outputln(self.prefix + "<%d invalid chars>"%len(text), recordType = self.recordType)
...
class String(BaseRecordHandler):
    ...
    def parseBytes (self):
        name = getTextBytes(self.readRemainingBytes())
...
class UniString(BaseRecordHandler):
    ...
    def parseBytes (self):
        name = getUTF8FromUTF16(getTextBytes(self.readRemainingBytes()))
        self.appendProperty(name)
        self.appendLine("text: '%s'"%name.decode('UTF-8'))
        self.appendText(name)
...
class ZipRecord(BaseRecordHandler):
    ...
    def parseBytes (self):
        outputZipContent(self.readRemainingBytes(), self.appendLine, 61)

class FontEntity(BaseRecordHandler):
    """Font entity."""

    def parseBytes (self):
        faceName       = getUTF8FromUTF16(getTextBytes(self.readBytes(64)))
        ...
        self.appendLine("Font: name=\"%s\" charset=%d flags=0x%x type=%d family=%d"%(nulltrunc(faceName).decode('cp1252'), charSet, flags, fontType, pitchAndFamily))
...
class ColorScheme(BaseRecordHandler):
    ...
    def parseBytes (self):
        self.appendLine(stringizeColorRef(self.readUnsignedInt(4), "Background"))
        self.appendLine(stringizeColorRef(self.readUnsignedInt(4), "Text and lines"))
        self.appendLine(stringizeColorRef(self.readUnsignedInt(4), "Shadows"))
        self.appendLine(stringizeColorRef(self.readUnsignedInt(4), "Title text"))
        self.appendLine(stringizeColorRef(self.readUnsignedInt(4), "Fills"))
        self.appendLine(stringizeColorRef(self.readUnsignedInt(4), "Accent"))
        self.appendLine(stringizeColorRef(self.readUnsignedInt(4), "Accent and hyperlink"))
        self.appendLine(stringizeColorRef(self.readUnsignedInt(4), "Accent and followed hyperlink"))
...
class AnimationInfo(BaseRecordHandler):
    ...
    def parseBytes (self):
        self.appendLine(stringizeColorRef(self.readUnsignedInt(4),
                                                  "DimColor"))
        flags = self.readUnsignedInt(4)
...
class AnimAttributeValue(BaseRecordHandler):
    ...
    def handleFloat (self):
        self.appendLine("float value: %f"%getFloat(self.readBytes(4)))

    def handleString (self):
        value = getUTF8FromUTF16(getTextBytes(self.readRemainingBytes()))
...
class AnimValue(BaseRecordHandler):
    ...
    def handleRepeat (self):
        self.appendLine("repeat count: %f"%getFloat(self.readBytes(4)))

    def handleAccelerate (self):
        self.appendLine("accelerate amount: %f"%getFloat(self.readBytes(4)))

    def handleDecelerate (self):
        self.appendLine("decelerate amount: %f"%getFloat(self.readBytes(4)))
...
class UniCharPropertyHandler(BasePropertyHandler):
    ...
    def output (self):
        if self.isComplex:
            name = getUTF8FromUTF16(getTextBytes(self.bytes))
...
class ColorPropertyHandler(BasePropertyHandler):
    ...
    def output (self):
        ...
        self.printer("%4.4Xh: %s [\"%s\"]"%(self.propType,
                                            stringizeColorRef(self.propValue,
                                                                      propEntry[0]),
                                            propEntry[2]))
...
class ZipStoragePropertyHandler(BasePropertyHandler):
    ...
    def output (self):
        outputZipContent(self.bytes, self.printer, 61)
