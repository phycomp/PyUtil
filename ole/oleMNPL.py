...
#import sys
#import pptGlobals
from pptGlobals import getSignedInt, output, indexbytes, outputln, nulltrunc, dumpBytes, OutputWidth, getSectorPos, PY3, getUTF8FromUTF16
# ----------------------------------------------------------------------------getSectorSize, getSectorIDChain, 
...
class Header(object):
    ...
    @staticmethod
    def byteOrder (chars):
        b1, b2 = indexbytes(chars,0), indexbytes(chars, 1)
    ...
    def output (self):
        ...
        printSep('=', OutputWidth)
        outputln("Compound Document Header")
        printSep('-', OutputWidth)

        #if self.params.debug:
        #    dumpBytes(self.bytes[0:512])
        ...
        outputln("Revision: %d  Version: %d"%(self.revision, self.version))

        # byte order
        output("Byte order: ")
        if self.byteOrder == ByteOrder.LittleEndian:
            outputln("little endian")
        ...
        outputln("Sector size: %d (%d)"%(2**self.secSize, self.secSize))

        # short sector size (usually 64 bytes)
        outputln("Short sector size: %d (%d)"%(2**self.secSizeShort, self.secSizeShort))

        # total number of sectors in SAT (equals the number of sector IDs
        # stored in the MSAT).
        outputln("Total number of sectors used in SAT: %d"%self.numSecSAT)

        outputln("Sector ID of the first sector of the directory stream: %d"%
              self.__secIDFirstDirStrm)

        outputln("Minimum stream size: %d"%self.minStreamSize)

        if self.__secIDFirstSSAT == -2:
            outputln("Sector ID of the first SSAT sector: [none]")
        else:
            outputln("Sector ID of the first SSAT sector: %d"%self.__secIDFirstSSAT)

        outputln("Total number of sectors used in SSAT: %d"%self.numSecSSAT)

        if self.__secIDFirstMSAT == -2:
            # There is no more sector ID stored outside the header.
            outputln("Sector ID of the first MSAT sector: [end of chain]")
        else:
            # There is more sector IDs than 109 IDs stored in the header.
            outputln("Sector ID of the first MSAT sector: %d"%(self.__secIDFirstMSAT))

        outputln("Total number of sectors used to store additional MSAT: %d"%self.numSecMSAT)
...
"""
    ...
    def output (self):
        #globals.outputln('')
        outputln("="*OutputWidth)
        outputln("Master Sector Allocation Table (MSAT)")
        outputln("-"*OutputWidth)

        for id in self.secIDs:
            outputln("sector ID: %5d   (pos: %7d)"%(id, 512+id*self.sectorSize))
    ...
    def outputArrayStats (self):
        ...
        for i in range(0, len(self.array)):
            ...
            if item >= 0:
                ...
                sectorLiveTotal += 1
        outputln("total sector count:          %4d"%sectorTotal)
        outputln("* live sector count:         %4d"%sectorP)
        outputln("* end-of-chain sector count: %4d"%sectorM2)  # end-of-chain is also live

        outputln("* free sector count:         %4d"%sectorM1)
        outputln("* SAT sector count:          %4d"%sectorM3)
        outputln("* MSAT sector count:         %4d"%sectorM4)
        outputln("* other sector count:        %4d"%sectorMElse)


    def output (self):
        outputln('')
        outputln("="*OutputWidth)
        outputln("Sector Allocation Table (SAT)")
        outputln("-"*OutputWidth)
        if self.params.debug:
            self.outputRawBytes()
            outputln("-"*OutputWidth)
            for i in range(0, len(self.array)):
                outputln("%5d: %5d"%(i, self.array[i]))
            outputln("-"*OutputWidth)
...
"""
    ...
    def output (self):
        outputln('')
        outputln("="*OutputWidth)
        outputln("Short Sector Allocation Table (SSAT)")
        outputln("-"*OutputWidth)
        if self.params.debug:
            self.outputRawBytes()
            outputln("-"*OutputWidth)
...
"""
    ...
    def output (self, debug=False):
        ...
        if debug:
            outputln("-"*OutputWidth)
            outputln("sector(s) used:")
            for secID in self.sectorIDs:
                outputln("  sector %d"%secID)

            outputln("")
            for secID in self.sectorIDs:
                outputln("-"*OutputWidth)
                outputln("  Raw Hex Dump (sector %d)"%secID)
                outputln("-"*OutputWidth)
                pos = getSectorPos(secID, self.sectorSize)
                dumpBytes(self.bytes[pos:pos+self.sectorSize], 128)
    ...
    def __outputEntry (self, entry, debug):
        outputln("-"*OutputWidth)
        if len(entry.Name) > 0 and indexbytes(entry.Name, 0) != 0:
            name = entry.Name
            # entry.Name : utf-8 bytes 
            if indexbytes(name, 0) <= 5:
                name = b"<%2.2Xh>%s"%(indexbytes(name, 0), name[1:])
            sname = nulltrunc(name).decode('utf-8')
            outputln("name: %s   (name buffer size: %d bytes)"%(sname, entry.CharBufferSize))
        else:
            outputln("name: [empty]   (name buffer size: %d bytes)"%entry.CharBufferSize)

        #if self.params.debug:
        #    outputln("-"*OutputWidth)
        #    dumpBytes(entry.bytes)
        #    outputln("-"*OutputWidth)
        ...
        if entry.NodeColor == Directory.NodeColor.Red:
            ...
            outputln("[unknown color]")

        outputln("linked dir entries: left: %d; right: %d; root: %d"%
        ...
        if entry.StreamSectorID < 0 or entry.StreamSize == 0:
            outputln("[empty stream]")
        else:
            strmLoc = "SAT"
            if entry.StreamLocation == StreamLocation.SSAT:
                strmLoc = "SSAT"
            outputln("(first sector ID: %d; size: %d; location: %s)"%
            ...
            if satObj != None:
                ...
                outputln("sector count: %d"%len(chain))
                outputln("total sector size: %d"%(len(chain)*secSize))
    ...
    def parseDirEntry (self, bytes):
        ...
        entry.bytes = bytes
        name = getUTF8FromUTF16(bytes[0:64])
        entry.Name = name
...
#            newEntry.HierachicalName = parent.HierachicalName + globals.encodeName( newEntry.Entry.Name )
    ...
    def __printListReport( self, treeNode ):
        ...
        if len( treeNode.HierachicalName ) > 0 :
            outputln('{0:8d}  {1:0<2d}-{2:0<2d}-{3:0<2d} {4:0<2d}:{5:0<2d}   {6}'.format(treeNode.Entry.StreamSize, dateInfo.day, dateInfo.month, dateInfo.year, dateInfo.hour, dateInfo.second, treeNode.HierachicalName ))
    ...
    def __printHeader(self):
        outputln("OLE: %s")%self.filePath
        outputln(" Length     Date   Time    Name")
        outputln("--------    ----   ----    ----")
    ...
    def extract(self, name):
        ...
        if  self.rootNode != None:
            ...
            outputln("failed to initialise ole container")
