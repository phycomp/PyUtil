...
#import sys pptGlobals, 
#from . import ole, 
import pptrecord
from ole import Header
from pptGlobals import output, outputln, getUnsignedInt, indexbytes
#ColorScheme, ObjectRefAtom, TextHeader, TextStyles, MasterTextStyles
...
class PPTFile(object):
    ...
    def printStreamInfo (self):
        self.__printSep('=', 68)
        outputln("PPT File Format Dumper by K. Yoshida, T. Behrens & contributors")
        outputln("  total stream size: %d bytes"%self.size)
        self.__printSep('=', 68)
        outputln('')
...
class PPTDirStream(object):
    ...
    def readUnsignedInt (self, size=1):
        return getUnsignedInt(self.readBytes(size))


    def __print (self, text, recordType = -1):
        outputln(self.prefix + text, recordType = recordType)
    ...
    def printRecordDump (self, bytes, recordType):
        ...
        for i in range(0, size):
            ...
            output("%2.2X "%indexbytes(bytes, i), recordType = recordType)
...
# skipping irrelevant records) ShapeString, ShapeUniString, DocAtom, SlideAtom, SlidePersistAtom, SlideInfo, ViewInfoAtom, SlideViewInfoAtom, ZipRecord
textRecData = {
